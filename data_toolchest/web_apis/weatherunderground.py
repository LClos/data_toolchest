"""
wu_returns = json_f
interested in json_f['history']['dailysummary'][0]['meandewptm']
"""
...
class Weather_Data_Dict:
    ...
    def _retrieve_local_records(self,records_dirpath):
        ...
        for datetag in json_datelt:
            ...
            if local_date_record == {}: continue
            else: local_records_dict[datetag] = local_date_record
    ...
    def _write_json(self,json_datetag,datedict):
        ...
        try:
            ...
            sys.stderr.write('<ERROR> Failure generating json file:%s\n'%json_fnpath)
            sys.exit(-1)
        else:
            # sys.stdout.write('<NOTICE> json file produced:%s\n'%json_fnpath)
            return True
...
if __name__=='__main__':
    ...
    # for datetag in sorted(wu_obj.datedict.keys(),reverse=True):
